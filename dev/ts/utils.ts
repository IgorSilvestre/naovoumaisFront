...
function numberToMoney (productPrice: number) {
...
function insertSpinner () {
    ...
    if (spinnerArea != null) spinnerArea.innerHTML = spinnerHTML
    else  console.log(`spinner area HTML div couldn't be fetched`)
}
...
async function insertProducts (data: { nextPage: number; products?: any }) {
    ...
    if (products.length == 0) {
        if (page > 0) {
            if (productCardArea) productCardArea.innerHTML += `<h3>Não há mais produtos para serem carregados</h3>`
                else console.log(`product card area HTML div coudn't be fetched`)
            document.querySelector('[more-products-button]')?.remove()
            return
        }
        if (productCardArea) productCardArea.innerHTML = `<h3>Nenhum produto encontrado, tente pesquisar por outra coisa!</h3>`
            else console.log(`product card area HTML div coudn't be fetched`)
        return
    }
    try {
        products.forEach((product: { description: string; name: string; price: number; city: string; date: string | Date }) => {
            let description = product.description
            if (description.length > 120) {
                product.description = description.slice(0, 120) + '...'
            }
            if (productCardArea) productCardArea.innerHTML += `
